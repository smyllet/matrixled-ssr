...
router.get("renderer", [
  () => import("#controllers/renderer_controller"),
  "render",
]);
...
router
  .resource("auth/logout", LogoutController)
  .only(["store"])
  .use("*", middleware.auth());
...
router
  .get("auth/email/verify", [EmailVerificationController, "verify"])
...
router
  ...
  .use(middleware.guest());
router
  .resource("matrices", MatricesController)
  .use("*", middleware.auth())
  .as("matrices");
